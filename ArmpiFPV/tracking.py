...
def getAreaMaxContour(contours):
    ...
    for c in contours:  # 历遍所有轮廓
        contour_area_temp = math.fabs(cv2.contourArea(c))  # 计算轮廓面积 #calc max contour area
        if contour_area_temp > contour_area_max:
            contour_area_max = contour_area_temp
            if contour_area_temp > 10:  # 只有在面积大于300时，最大面积的轮廓才是有效的，以过滤干扰 #not sure what this means, but largest contour chosen here.
                area_max_contour = c
...
def run(img):
    ...
    frame_lab = cv2.cvtColor(frame_resize, cv2.COLOR_BGR2LAB)  # 将图像转换到LAB空间 #convert img to LAB space
    ...
    if area_max > 100:  # 有找到最大面积 #largest area
        (center_x, center_y), radius = cv2.minEnclosingCircle(area_max_contour)  # 获取最小外接圆 #smallest circumcircle
